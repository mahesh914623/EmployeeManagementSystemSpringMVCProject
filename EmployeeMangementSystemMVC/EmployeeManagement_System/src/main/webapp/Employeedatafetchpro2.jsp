...
<body>
	<header id="seccontainer">
		...
		<span style=" margin-left:450px;color: white;font-size: 30px;"><i class="fa-solid fa-right-from-bracket"></i><a href="Managerlogin.jsp" style="text-decoration: none; color: white">Logout</a></span>
	</header>
	<div class="container">
		<div id="emp1">
			<i id="employee" class="fa-solid fa-user-tie"   title="manger"></i><br>
			<a id="employee_id" href="">Maneger</a><br>
		</div>
...
<c:forEach var="emp" items="${data_l}"> 
<tr>
<td>${emp.id }</td>
<td>${emp.name }</td>
<td>${emp.email }</td>
...
<td>${emp.role }</td>
<td>${emp.phone }</td>
<td> <form action="/salary/${emp.id}" method="post"><select><input type="text" name="salary"></input><button type="submit">Save</button></select></form></td>
<td><a class="del" href="<c:url value='/delete1/${emp.id}'/>">Delete</a></td>
<td><a class="del1" href="<c:url value='/${emp.id}'/>">Edit</a></td>
